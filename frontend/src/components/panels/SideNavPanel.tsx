  ...
  return (
    <AnimatePresence mode="wait">
      <motion.div
        ...
        className={`h-full bg-card flex flex-col overflow-hidden shadow-lg border-r border-border ${styles.navPanel}`}
      >
        {/* Sticky header for mobile and desktop */}
        <motion.div
          className={`sticky top-0 z-10 bg-card/95 border-b border-border flex-shrink-0 px-4 py-3 md:py-4 ${styles.stickyHeader}`}
          initial={{ opacity: 0, y: -10 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.3 }}
        >
        ...
        <div className="flex-1 min-h-0 overflow-hidden">
          <div
            className={`h-full seamless-scroll overflow-y-auto px-2 md:px-4 py-2 md:py-4 ${styles.scrollArea}`}
          >
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
